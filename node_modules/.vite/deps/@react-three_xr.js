import {
  GLTFLoader
} from "./chunk-DEXTMGSP.js";
import {
  MeshLineGeometry,
  MeshLineMaterial,
  require_with_selector,
  tunnel
} from "./chunk-GI4Y4HSO.js";
import {
  require_client
} from "./chunk-6IPQHY7T.js";
...
async function loadXRHandModel(assetPath, loader = DefaultGltfLoader) {
...
function createXRHandState(inputSource, options, events) {
  return {
    type: "hand",
    inputSource,
...
function createSyncXRInputSourceStates(addAsyncMap, options) {
  let currentInputSources = /* @__PURE__ */ new Set();
  ...
  return (session, currentStates, added, removed) => {
    ...
    if (removed === "all") {
      ...
      for (let i2 = 0; i2 < removedLength; i2++) {
        const inputSource = removed[i2];
...
function bindXRInputSourceEvent(session, inputSource, event, fn) {
  const filterFn = (event2) => {
    if (event2.inputSource != inputSource) {
      return;
    }
    fn(event2);
  };
...
function buildXRSessionInit(mode, domOverlayRoot, { anchors = true, handTracking = true, layers = true, meshDetection = true, planeDetection = true, originReferenceSpace = "local-floor", customSessionInit, depthSensing = false, hitTest = true, unbounded = true, domOverlay = true } = {}) {
...
function resolveDetectedImplementation(implementation, semanticLabel, defaultValue) {
  implementation ?? (implementation = defaultValue);
  if (implementation === false) {
    return false;
  }
  if (typeof implementation === "function") {
...
function startEmulate(emulate, alert) {
  var _a, _b;
  Promise.all([(_a = navigator.xr) == null ? void 0 : _a.isSessionSupported("immersive-vr"), (_b = navigator.xr) == null ? void 0 : _b.isSessionSupported("immersive-ar")]).then(([vr, ar]) => !ar && !vr ? import("./emulate-LRGZV2NK.js") : void 0).then((pkg) => {
    if (alert) {
...
function createXRStore(options) {
  ...
  const store = createStore(() => ({
    ...
    detectedMesh: options == null ? void 0 : options.detectedMesh,
    detectedPlane: options == null ? void 0 : options.detectedPlane,
    domOverlayRoot
  }));
...
function createBindToSession(store, syncXRInputSourceStates) {
  ...
  return (session, mode) => {
    ...
    store.setState({
      ...syncXRInputSourceStates(session, void 0, session.inputSources, void 0),
    ...
    cleanupSession = () => {
      syncXRInputSourceStates(session, store.getState(), void 0, "all");
...
function updateSession(store, frame, manager) {
  ...
  for (let i2 = 0; i2 < controllersLength; i2++) {
    updateXRControllerState(controllers[i2]);
  }
  const handsLength = hands.length;
  for (let i2 = 0; i2 < handsLength; i2++) {
    updateXRHandState(hands[i2], frame, manager);
  }
}
var emptyArray = [];
...
function isFacingCamera(camera, object, direction, angle) {
  ...
  camera.getWorldPosition(vectorHelper5);
  vectorHelper5.sub(positionHelper);
  return vectorHelper5.angleTo(directionHelper2) < angle / 2;
}
...
async function createXRHitTestSource(store, relativeTo, trackableType = ["point", "plane", "mesh"]) {
  ...
  if (relativeTo instanceof XRSpace) {
    ...
    object = relativeTo;
    space = getSpaceFromAncestors(relativeTo, state.origin, state.originReferenceSpace, matrixHelper3) ?? state.originReferenceSpace;
    if (space === state.originReferenceSpace) {
      computeOriginReferenceSpaceOffset(relativeTo, state.origin, matrixHelper3);
    }
    vectorHelper6.setFromMatrixPosition(matrixHelper3);
    const point = { ...vectorHelper6 };
    quaternionHelper5.setFromRotationMatrix(matrixHelper3);
    vectorHelper6.set(0, 0, -1).applyQuaternion(quaternionHelper5);
    const direction = { ...vectorHelper6 };
...
var ZeroVector = new Vector3(1, 1, 1);
...
var directionHelper3 = new Vector3();
async function requestXRAnchor(store, options) {
  var _a, _b, _c;
  if (options.relativeTo === "hit-test-result") {
    directionHelper3.set(0, 0, 1);
    if (options.offsetQuaternion != null) {
      directionHelper3.applyQuaternion(options.offsetQuaternion);
    }
    return (_b = (_a = options.hitTestResult).createAnchor) == null ? void 0 : _b.call(_a, new XRRigidTransform({ ...options.offsetPosition }, { ...directionHelper3 }));
  }
  let frame;
  let space;
  if (options.relativeTo === "world") {
    ...
    if (origin != null) {
      ...
      matrixHelper22.decompose(positionHelper2, quaternionHelper6, directionHelper3);
  ...
  directionHelper3.set(0, 0, 1);
  directionHelper3.applyQuaternion(quaternionHelper6);
  return (_c = frame.createAnchor) == null ? void 0 : _c.call(frame, new XRRigidTransform({ ...positionHelper2 }, { ...directionHelper3 }), space);
}
...
var XRControllerLayoutLoader = class {
  constructor(options) {
    ...
    //cache
    __publicField(this, "profilesListPromise");
    __publicField(this, "profilePromisesMap", /* @__PURE__ */ new Map());
  ...
  async loadProfile(inputSourceProfileIds) {
    this.profilesListPromise ?? (this.profilesListPromise = fetchJson(new URL("profilesList.json", this.baseAssetPath).href));
    const profilesList = await this.profilesListPromise;
    const length = inputSourceProfileIds.length;
    for (let i2 = 0; i2 < length; i2++) {
      const profileInfo2 = profilesList[inputSourceProfileIds[i2]];
      if (profileInfo2 == null) {
        continue;
      }
      return this.loadProfileFromPathCached(profileInfo2.path);
    }
...
async function createXRControllerState(inputSource, layoutLoader, events) {
...
var xrMeshContext = (0, import_react.createContext)(void 0);
var xrPlaneContext = (0, import_react.createContext)(void 0);
var xrInputSourceStateContext = (0, import_react.createContext)(void 0);
var xrReferenceSpaceContext = (0, import_react.createContext)(void 0);
...
function useSessionSupported(mode, onError) {
...
var XRControllerComponent = (0, import_react4.forwardRef)(({ id, children, onPress, onRelease }, ref) => {
  ...
  useXRControllerButtonEvent(id, (state2) => state2 === "pressed" ? onPress == null ? void 0 : onPress() : onRelease == null ? void 0 : onRelease());
  useFrame(() => {
    var _a;
...
function useXRControllerButtonEvent(id, onChange, handedness) {
...
function useXRControllerState(handedness) {
  ...
  const context2 = (0, import_react4.useContext)(xrInputSourceStateContext);
  if (context2 == null || context2.type != "controller") {
...
function useXRHandState(handedness) {
  if (handedness != null) {
    return useXR((s) => s.handStates.find((state2) => state2.inputSource.handedness === handedness));
  }
  const state = (0, import_react5.useContext)(xrInputSourceStateContext);
...
var LoadXRHandModelSymbol = Symbol("loadXRHandModel");
var XRHandModel2 = (0, import_react5.forwardRef)((options, ref) => {
  const state = useXRHandState();
  const model = suspend(loadXRHandModel, [state.assetPath, void 0, LoadXRHandModelSymbol]);
...
function CombinedPointer2({ children }) {
  const pointer = (0, import_react6.useMemo)(() => new CombinedPointer(), []);
...
function useSetupPointer(pointer, makeDefault = false) {
  ...
  if (combinedPointer == null) {
    ...
    (0, import_react6.useEffect)(() => combinedPointer.register(pointer, makeDefault), [combinedPointer, pointer, makeDefault]);
  }
  (0, import_react6.useEffect)(() => () => pointer.exit({ timeStamp: performance.now() }), [pointer]);
}
function usePointerXRSessionVisibility(pointer) {
  const store = useXRStore();
  (0, import_react6.useEffect)(() => setupSyncIsVisible(store, (visible) => pointer.setEnabled(visible, { timeStamp: performance.now() })), [store, pointer]);
}
...
function XRElements({ children }) {
  ...
  return (0, import_jsx_runtime6.jsx)(import_jsx_runtime6.Fragment, { children: reconciler.createPortal((0, import_jsx_runtime6.jsx)(context.Provider, { value: store, children: (0, import_jsx_runtime6.jsxs)(xrReferenceSpaceContext.Provider, { value: referenceSpace, children: [(0, import_jsx_runtime6.jsxs)("group", { matrixAutoUpdate: false, visible, children: [(0, import_jsx_runtime6.jsx)(XRControllers, {}), (0, import_jsx_runtime6.jsx)(XRHands, {}), (0, import_jsx_runtime6.jsx)(XRTransientPointers, {}), (0, import_jsx_runtime6.jsx)(XRGazes, {}), (0, import_jsx_runtime6.jsx)(XRScreenInputs, {})] }), (0, import_jsx_runtime6.jsx)(XRDetectedMeshes, {}), (0, import_jsx_runtime6.jsx)(XRDetectedPlanes, {}), children] }) }), storeWithOriginAsScene, null) });
}
...
function XRDetectedMeshes() {
  ...
  return (0, import_jsx_runtime6.jsx)(import_jsx_runtime6.Fragment, { children: meshes.map((mesh) => {
    const ResolvedImpl = resolveDetectedImplementation(Implementation, mesh.semanticLabel, false);
    if (ResolvedImpl === false) {
      return null;
    }
    return (0, import_jsx_runtime6.jsx)(XRSpace3, { space: mesh.meshSpace, children: (0, import_jsx_runtime6.jsx)(xrMeshContext.Provider, { value: mesh, children: (0, import_jsx_runtime6.jsx)(import_react9.Suspense, { children: (0, import_jsx_runtime6.jsx)(ResolvedImpl, {}) }) }) }, objectToKey(mesh));
  }) });
}
function XRDetectedPlanes() {
  ...
  return (0, import_jsx_runtime6.jsx)(import_jsx_runtime6.Fragment, { children: planes.map((plane) => {
    const ResolvedImpl = resolveDetectedImplementation(Implementation, plane.semanticLabel, false);
    if (ResolvedImpl === false) {
      return null;
    }
    return (0, import_jsx_runtime6.jsx)(XRSpace3, { space: plane.planeSpace, children: (0, import_jsx_runtime6.jsx)(xrPlaneContext.Provider, { value: plane, children: (0, import_jsx_runtime6.jsx)(import_react9.Suspense, { children: (0, import_jsx_runtime6.jsx)(ResolvedImpl, {}) }) }) }, objectToKey(plane));
  }) });
}
...
function useXR(selector = (state) => state) {
  return useStore2(useXRStore(), selector);
}
...
var XRSpace3 = (0, import_react11.forwardRef)(({ space, children }, ref) => {
  ...
  return (0, import_jsx_runtime8.jsx)("group", { xrSpace: space, visible: false, matrixAutoUpdate: false, ref: internalRef, children: (0, import_jsx_runtime8.jsx)(xrReferenceSpaceContext.Provider, { value: space, children }) });
});
function useXRReferenceSpace() {
  const context2 = (0, import_react11.useContext)(xrReferenceSpaceContext);
  if (context2 == null) {
...
function useGetXRSpaceMatrix(space) {
  const localReferenceSpace = (0, import_react11.useContext)(xrReferenceSpaceContext);
  const referenceSpace = useXR((xr) => localReferenceSpace ?? xr.originReferenceSpace);
  return (0, import_react11.useMemo)(() => referenceSpace == null ? void 0 : createGetXRSpaceMatrix(space, referenceSpace), [space, referenceSpace]);
}
...
var XRMeshModel = (0, import_react12.forwardRef)((props, ref) => {
  const mesh = useXRMesh();
  const geometry = useXRMeshGeometry(mesh);
  return (0, import_jsx_runtime9.jsx)("mesh", { ref, geometry, ...props });
});
function useXRMesh() {
  const context2 = (0, import_react12.useContext)(xrMeshContext);
  if (context2 == null) {
...
var XRPlaneModel = (0, import_react13.forwardRef)((props, ref) => {
  const plane = useXRPlane();
  const geometry = useXRPlaneGeometry(plane);
  return (0, import_jsx_runtime10.jsx)("mesh", { ref, geometry, ...props });
});
function useXRPlane() {
  const context2 = (0, import_react13.useContext)(xrPlaneContext);
  if (context2 == null) {
...
function useXRTransientPointerState(handedness) {
  if (handedness != null) {
    return useXR((s) => s.transientPointerStates.find((state2) => state2.inputSource.handedness === handedness));
  }
  const state = (0, import_react15.useContext)(xrInputSourceStateContext);
...
var XROrigin = (0, import_react16.forwardRef)(({ children, ...props }, ref) => {
  ...
  return (0, import_jsx_runtime15.jsxs)("group", { ref, ...props, children: [(0, import_jsx_runtime15.jsx)("primitive", { object: xrCamera }), (0, import_jsx_runtime15.jsx)(xrReferenceSpaceContext.Provider, { value: referenceSpace, children })] });
});
...
var XRHitTest = (0, import_react17.forwardRef)((props, ref) => {
  ...
  return (0, import_jsx_runtime16.jsx)("group", { ...props, ref: internalRef });
});
...
function useXRAnchor() {
  ...
  const store = useXRStore();
  const create = (0, import_react18.useCallback)(async (options) => {
    var _a;
...
var XRButton = (0, import_react20.forwardRef)(({ store, mode, onError, children, ...props }, ref) => {
  ...
  return (0, import_jsx_runtime18.jsx)("button", { ref, ...props, onClick: () => session != null ? session.end() : store.enterXR(mode).catch(onError), children: typeof children === "function" ? children(supported ? session != null ? "entered" : "exited" : "unsupported") : children });
});
var ARButton = (0, import_react20.forwardRef)((props, ref) => {
  return (0, import_jsx_runtime18.jsx)(XRButton, { ref, mode: "immersive-ar", ...props });
});
var VRButton = (0, import_react20.forwardRef)((props, ref) => {
  return (0, import_jsx_runtime18.jsx)(XRButton, { ref, mode: "immersive-vr", ...props });
});
...
var Interactive = (0, import_react22.forwardRef)(({ onHover, onBlur, onSelectStart, onSelectEnd, onSelect, onSqueezeStart, onSqueezeEnd, onSqueeze, onMove, children }, passedRef) => {
  ...
  return (0, import_jsx_runtime19.jsx)("group", { ref, children });
});
...
var RayGrab = (0, import_react23.forwardRef)(function RayGrab2({ onSelectStart, onSelectEnd, children, ...rest }, forwardedRef) {
  ...
  return (0, import_jsx_runtime20.jsx)(Interactive, { ref: groupRef, onSelectStart: (e) => {
